...
def create_bert_dataset(device_num=1, rank=0, rbd_name="alpha", do_shuffle="true", data_dir=None, schema_dir=None,
                        batch_size=32,
                        bucket_list=None):
    """create train dataset"""
    data_files = glob.glob(
        data_dir + "/s_dedup_%s*.txt.mindrecord" % rbd_name)  # s_dedup_omicron_train_11.txt.mindrecord
    print("Train data:>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
    print(data_files)
    data_files.sort()
